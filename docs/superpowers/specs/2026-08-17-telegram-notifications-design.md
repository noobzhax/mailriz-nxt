...
### `/refresh` command — webhook (2026-08-17)

The bot gains one command: `/refresh` forces the open dashboard tab to
refetch its inbox immediately. This is the first webhook — previously
deferred scope.

- **Endpoint**: `POST /api/telegram/webhook`, mounted **before** `jwtAuth`
  (public). Every update is verified against `X-Telegram-Bot-Api-Secret-Token`
  (timing-safe compare against the stored secret).
- **Secret**: generated by the Worker on first registration and stored in the
  settings row (`telegram_webhook_secret`) — low-value credential, D1 is fine.
- **Registration**: a dashboard Settings button calls a Worker endpoint that
  performs `setWebhook` (`https://{dashboard}/api/telegram/webhook`) and
  `setMyCommands` (so `/refresh` shows in the bot's "/" menu). The Worker has
  the bot token secret, so no CLI change is needed for the webhook itself.
- **Handler**: text `/refresh` (any chat in the configured list) →
  `UPDATE settings SET telegram_refresh_at = <now>` → reply
  "🔄 Memeriksa inbox…". Everything else is ignored (200).
- **Signalling the dashboard**: the SSE stream (`routes/updates.ts`) now also
  polls `telegram_refresh_at`; when it advances past the connection's
  baseline it emits `event: refresh` with id `refresh:{ts}`. The dashboard
  refetches on `refresh` the same way it does on `mail`.
- **Cloudflare Access installs**: the dashboard hostname sits behind Access,
  which would challenge Telegram's server. The CLI's `ensureAccessApp` gains
  a second **bypass** policy scoped to the path `/api/telegram/webhook`
  (path-scoped policies on the existing application). Session-mode installs
  need nothing — the route is already outside `jwtAuth`.
- **Out of scope, still**: mark-read / archive actions, per-chat filters,
  reply-from-Telegram.